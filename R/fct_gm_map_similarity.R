...
#'
#' `gm_map_similarity()` creates a map of similarity scores from the results of running [gm_crossmatch()].
#' The map is a `ggplot()` object.
#'
...
gm_map_similarity <- function(geom1, geom2, id1, id2, name1, name2, table,
  ...
  if (plot %in% c("forward", "f")) {
    ...
    p <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = g, ggplot2::aes(fill = s), colour = "#000000", lwd = lwd) +
    ...
    if (labels %in% c("id", "name")) {
      ...
      p <- p + ggrepel::geom_label_repel(ggplot2::aes(label = paste0(!!rlang::sym(labels), "\n", round(.data$s,2)), geometry = .data$geometry),
                                         size = size, color = "red", stat = "sf_coordinates",
                                         min.segment.length = 0, segment.color = "red",
                                         max.overlaps = Inf, alpha = a, seed = 519, force = f,
                                         data = sf::st_centroid(g |> dplyr::filter(dplyr::between(.data$s, label.range[1], label.range[2])))) +
  ...
  if (plot %in% c("backward", "b")) {
    ...
    p <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = g, ggplot2::aes(fill = .data$s), colour = "#000000", lwd = lwd) +
      viridis::scale_fill_viridis(direction = -1, limits = c(lim,1))  +
  ...
  if (plot %in% c("combined", "c")) {
    ...
    geom_1 <- geom1 |> dplyr::rename(id = !!rlang::sym(id1)) |> dplyr::left_join(table[['similarity']] |> dplyr::filter(.data$rank_areal_f == 1) |> dplyr::select(id1, s), by = dplyr::join_by(id == id1))
    geom_2 <- geom2 |> dplyr::rename(id = !!rlang::sym(id2)) |> dplyr::left_join(table[['similarity']] |> dplyr::filter(.data$rank_areal_b == 1) |> dplyr::select(id2, s), by = dplyr::join_by(id == id2))
    ...
    lim = min(c(min(geom_1$s), min(geom_2$s)))

    p1 <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = geom_2, fill = NA, color = NA) +
      ggplot2::geom_sf(data = geom_1, ggplot2::aes(fill = .data$s), colour = "grey50", lwd = lwd) +
      viridis::scale_fill_viridis(direction = -1, limits = c(lim,1))  +
    ...
    p2 <- ggplot2::ggplot() +
      ...
      ggplot2::geom_sf(data = geom_2, ggplot2::aes(fill = .data$s), colour = "grey50", lwd = lwd) +
      viridis::scale_fill_viridis(direction = -1, limits = c(lim,1))  +
    ...
    if (labels %in% c("id", "name")) {
      ...
      p1 <- p1 + ggrepel::geom_label_repel(ggplot2::aes(label = paste0(!!rlang::sym(labels), "\n", round(s,2)), geometry = .data$geometry),
                                           size = size, color = "red", stat = "sf_coordinates",
                                           min.segment.length = 0, segment.color = "red",
                                           max.overlaps = Inf, alpha = a, seed = 519, force = f,
                                           data = sf::st_centroid(geom_1 |> dplyr::filter(dplyr::between(.data$s, label.range[1], label.range[2])))) +
        ggplot2::labs(caption = paste0("Labels displayed between ", label.range[1], " and ", label.range[2]))
      ...
      p2 <- p2 + ggrepel::geom_label_repel(ggplot2::aes(label = paste0(!!rlang::sym(labels), "\n", round(s,2)), geometry = .data$geometry),
                                         ...
                                         min.segment.length = 0, segment.color = "red",
                                         max.overlaps = Inf, alpha = a, seed = 519, force = f,
                                         data = sf::st_centroid(geom_2 |> dplyr::filter(dplyr::between(.data$s, label.range[1], label.range[2])))) +
        ggplot2::labs(caption = paste0("Labels displayed between ", label.range[1], " and ", label.range[2]))
    }
